...
**Phase 2 precision (2026-07-21):** manifest-aware **reachability grading** shipped (`mcp_scanner/reachability.py` + `mcp_scanner/tool_registry.py`). A post-detector pass discovers the registered MCP tools (`@mcp.tool()`/`server.tool(...)` decorators and any `server.json` manifest — the tool-scope-creep detector's decorator parser was extracted into the shared `tool_registry` module rather than duplicated) and walks a static call-graph, labelling every finding `reachable` / `unreachable-by-tools` / `unknown` and nudging confidence up/down accordingly (never dropping a finding). Same-file call-graph is exact; cross-file is best-effort by function name. This is item 3 below, delivered with stated limits — it labels reachability, not the individual tainted value (item 1 remains open).

**Still needed before it's a sellable SaaS:**

1. **Cross-file dataflow / taint** — reachability grading (Phase 2) now labels whether a finding is reachable from a tool via the static call-graph, but does not yet trace the individual tainted value tool-arg → sink to further lift confidence and cut P2/low noise.
2. **Deeper JS/TS coverage** — regex-level today; AST-parity with the Python path (JS findings are labelled reachability-`unknown` until then).
3. ~~**MCP-manifest awareness**~~ — **DONE (2026-07-21)**: `server.json` + `@mcp.tool()`/`server.tool(...)` discovery feeds the reachability grader that maps which findings are reachable from a registered tool. Full tool-parameter-schema → sink taint tracing folds into item 1.
