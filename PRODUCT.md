...
1. **Deep / multi-hop cross-file taint** — taint tracking v1 (Phase 3) traces a tool parameter into a sink same-file transitively and one import hop cross-file. Still open: a second import hop and beyond, cross-repo flow, and sanitizer-aware propagation (today a validated value is still treated as tainted, by design) — further precision to cut P2/low noise.
