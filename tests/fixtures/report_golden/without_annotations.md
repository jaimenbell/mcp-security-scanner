...
The scanner produced 3 raw finding(s) across 14 scanned file(s) in `acme-mcp`. No triage annotations were supplied: every finding below is UNREVIEWED. Treat this as the scanner's raw, deliberately over-flagging output, not a human-verified risk list.
...
1. **[P0/Unreviewed]** `srv/tools.py:42` -- a tool handler that passes caller-influenced input into a dangerous sink (shell, eval, unsafe deserialization, or an unguarded fetch/file path) (`2de1378d5d7a`).
