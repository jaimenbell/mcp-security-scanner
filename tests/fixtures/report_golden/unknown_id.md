...
| Triage annotations | triage_unknown.toml |
| Reviewed by | Jaimen |

## 2. Executive summary

The scanner produced 3 raw finding(s) across 14 scanned file(s) in `acme-mcp`. After human triage, 1 are confirmed real, 0 were dismissed as false positives, 0 accepted as risk, 0 already mitigated, and 2 remain unreviewed. 1 confirmed critical/high finding(s) require action.
...
2. **[P1/Unreviewed]** `srv/tools.py:88` -- a mutating MCP tool registered with no visible permission-group gate or env-flag opt-in (`ef3a4cf8b6b2`).
3. **[P2/Unreviewed]** `tests/conftest.py:7` -- a secret committed to the tree, hardcoded in source, or passed to a log/print call (`e28dfff3c483`).
...
> [!warning] Stale triage annotations -- action needed
> The annotations file references finding ids that are NOT in this scan (the finding may have been fixed, or the file renamed). They are listed here rather than silently dropped:
>
> - `aaaabbbbcccc` -- False positive: This finding was fixed in the previous engagement.
...
- Annotations file: `triage_unknown.toml`
- Annotations applied to findings in this scan: 1
- Annotations referencing unknown ids: 1
- Reviewers: Jaimen
